...
<?php
  if(isset($_POST['add'])){
   ...
   if(find_by_groupName($_POST['group-name']) === false ){
     $session->msg('d','<b>Error!</b> El nombre del Rol realmente existe en la base de datos');
   ...
   if(empty($errors)){
        ...
        if($db->query($query)){
          //sucess
          $session->msg('s',"El Rol ha sido creado! ");
          redirect('add_group.php', false);
        } else {
          //failed
...
<div class="login-page">
    <div class="text-center">
       <h3>Agregar nuevo Rol de usurio</h3>
     </div>
